...
`openctp-ctp` PyPI 包安装后，其底层 C++ 动态库 (`libthostmduserapi_se.so`, `libthosttraderapi_se.so`) 默认对接的是 **SimNow 仿真环境**。连接实盘环境，需要手动将其替换成招行的版本

仓库内提供了一个辅助脚本，可将 `ctp_package/` 中的 CTP 动态库覆盖到当前 `.venv`：

```bash
git-lfs pull # download .so
