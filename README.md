...
代码中通过环境变量 `XT_SDK_PATH` 控制 SDK 路径，默认值为 `~/xt_sdk`（见 `adapters/xuntou/base_service.py`）：
...
> **环境变量说明**：`XT_SDK_PATH` 允许在不同部署环境中灵活指定 SDK 位置，无需修改源码。使用 `scripts/setup_env.sh` 统一设置。
...
XunTou SDK 要求运行时 `LD_LIBRARY_PATH` 包含 SDK 路径，否则 `libXtTraderApi.so` 无法找到其 Boost/ICU 等依赖。

**但是**，这个路径**会污染 CTP 进程**（见下文隔离章节）。推荐使用 `scripts/setup_env.sh` 统一管理。
...
- 如果 `LD_LIBRARY_PATH` 包含 xt_sdk 路径，CTP 的 `_thostmduserapi.so` 加载时会链接到错误版本的共享库
- 结果：**进程启动即段错误 (SIGSEGV, exit code 139)**

### 解决方案：`setup_env.sh` + 进程级隔离

#### 统一环境设置脚本

`scripts/setup_env.sh` 负责统一设置所有 SDK 相关环境变量：
...
# 仅检查环境，不修改
source scripts/setup_env.sh --check
```
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `XT_SDK_PATH` | `~/xt_sdk` | 迅投 SDK 安装路径 |
| `LD_LIBRARY_PATH` | 自动追加 `XT_SDK_PATH` | 动态链接库搜索路径 |

#### 进程级 exec 跳板（`__main__` 守卫）

各服务节点在 `if __name__ == "__main__":` 中执行环境隔离，**不在模块 import 时产生副作用**：

**Market Node (CTP 行情进程) — 净化 xt_sdk 污染**：
...
**Trading Node / Xt Market Node (迅投进程) — 注入 xt_sdk 路径**：
...
> **设计要点**：bootstrap 放在 `__main__` 守卫下，而非模块顶层。这样 `import services.run_market_node` 不会触发进程重启，测试和工具代码可以安全导入模块中的函数。
...
**绝不要**在 `~/.bashrc` 中全局 export `LD_LIBRARY_PATH`。使用 `scripts/setup_env.sh` 按需设置：
...
如果在 bashrc 中全局 export，所有新 shell 启动的 CTP 进程都会段错误。
...
# 2. 如果要连生产 CTP，替换底层 .so（仅首次或 uv sync 后执行）
git lfs pull
bash scripts/swap_prod_ctp_so.sh
...
> **`setup_env.sh` 和 `swap_prod_ctp_so.sh` 不冲突**：前者设置 Shell 环境变量（`XT_SDK_PATH`、`LD_LIBRARY_PATH`），后者替换 `.venv` 中的 CTP `.so` 二进制文件。两者作用于不同层面，无执行顺序依赖，但建议先 swap 再 setup 以保持操作清晰。

### 启动顺序

系统由**多个独立进程**组成，必须分别在不同终端启动：

```bash
# 终端 1：启动 CTP 期货行情节点（必须先启动）
source scripts/setup_env.sh
uv run -m services.run_market_node
...
>
> **Xt Market Node** (`run_xuntou_market_node.py`) 是独立的迅投股票行情节点，用于接入 A 股实时行情并通过 Redis 转发。
...
│
├── services/                   # 微服务入口
│   ├── run_market_node.py      # CTP 期货行情节点 (CTP → Redis)
│   ├── run_xuntou_market_node.py  # 迅投股票行情节点 (XunTou → Redis)
│   ├── run_web_node.py         # Web 节点 (Flask + 迅投)
│   └── run_trading_node.py     # 交易节点 (Redis → 策略 → 迅投)
│
├── scripts/                    # 运维脚本
│   ├── setup_env.sh            # 统一环境变量设置 (XT_SDK_PATH, LD_LIBRARY_PATH)
│   ├── swap_prod_ctp_so.sh     # 切换 CTP .so 至生产环境
│   └── swap_openctp_so.sh      # 切换 CTP .so 至仿真环境
│
