...
├── __init__.py                              # 版本 6.7.10.0
├── thostmduserapi.py                        # Python 行情接口
├── _thostmduserapi.so                       # ✅ 已替换为 openctp 7x24 仿真版本
├── libthostmduserapi_se-da226578.so         # ✅ 已替换
├── thosttraderapi.py                        # Python 交易接口
├── _thosttraderapi.so                       # ✅ 已替换
└── libthosttraderapi_se-6124cc43.so         # ✅ 已替换
```

> **替换来源**：从 [OpenCTP 官方 GitHub Releases](https://github.com/openctp/openctp-ctp-python/releases) 下载对应 Python 3.13 + Linux x86_64 的 7x24 仿真版本 `.so` 文件，覆盖 `site-packages/openctp_ctp/` 目录下的原始文件。
...
| flow_path | `./ctp_flow/` | CTP SDK 会话文件存储目录 |
