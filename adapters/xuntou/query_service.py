        ...
        try:
            ...
            if error.isSuccess():
                ...
                logger.info(
                    ...
                    account_id, available, data.m_dBalance,
                )
                return available
            else:
    ...
    def query_today_orders(self, account_id: Optional[str] = None) -> List[OrderDetail]:
        ...
        try:
            ...
            if not error.isSuccess():
                logger.error("查询当日委托失败: account_id=%s, 错误=%s", account_id, error.errorMsg())
                return []
            ...
            if orders:
                ...
                logger.info("当日委托查询成功: account_id=%s, 共 %d 笔", account_id, len(result))
            else:
