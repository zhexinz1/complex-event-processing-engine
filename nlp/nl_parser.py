...
try:
    ...
    logger.warning(
        "Anthropic SDK not available. Natural language parsing will be disabled."
    )
...
def parse_natural_language(
    text: str, api_key: Optional[str] = None, model: str = "claude-3-5-sonnet-20241022"
) -> dict[str, Any]:
    """
    ...
    if not ANTHROPIC_AVAILABLE:
        raise ValueError(
            "Anthropic SDK not installed. Please install it with: pip install anthropic"
        )
