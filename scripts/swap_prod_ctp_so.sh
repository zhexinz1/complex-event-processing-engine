#!/usr/bin/env bash
# 将生产环境 CTP 原生 .so 覆盖到当前项目 .venv 中。
# 固定从仓库内的 ctp_prod_package/ 读取。
# 用法:
#   scripts/swap_prod_ctp_so.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
SOURCE_ROOT="${REPO_ROOT}/ctp_prod_package"

if [[ $# -ne 0 ]]; then
    echo "错误: 本脚本不接受参数，请直接执行 scripts/swap_prod_ctp_so.sh。"
    exit 1
fi
...
BACKUP_DIR="${BACKUP_ROOT}/${TIMESTAMP}_prod"
